...
{
	internal sealed class ItemReplyAllPreFormAction : IPreFormAction
	{
		public PreFormActionResponse Execute(OwaContext owaContext, out ApplicationElement applicationElement, out string type, out string state, out string action)
		{
			...
			try
			{
				...
				{
					throw new OwaInvalidRequestException("Item is not supported for reply all");
				}
				if (item is ReportMessage)
				{
					Utilities.TransferToErrorPage(owaContext, LocalizedStrings.GetNonEncoded(2128562495));
				}
				item3 = ReplyForwardUtilities.CreateReplyAllItem(BodyFormat.TextPlain, item, ReplyForwardFlags.None, userContext, null);
