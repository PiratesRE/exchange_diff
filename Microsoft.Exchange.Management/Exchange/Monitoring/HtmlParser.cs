...
namespace Microsoft.Exchange.Monitoring
{
	internal static class HtmlParser
	{
		public static string GetElement(string stringToSearch, string tagName, string attributeName, string attributeValue)
		{
			string text;
			string text2;
			return HtmlParser.GetElement(stringToSearch, tagName, attributeName, attributeValue, out text, out text2);
		}
		...
		public static string GetElement(string stringToSearch, string tagName, string attributeName, string attributeValue, out string innerHtml, out string parsedAttributeValue)
		{
			...
			innerHtml = null;
			string result = null;
			Match match = HtmlParser.MatchStartTag(stringToSearch, tagName, attributeName, attributeValue, out parsedAttributeValue);
			if (match.Success)
			{
				if (string.IsNullOrEmpty(tagName))
				{
					Match match2 = Regex.Match(stringToSearch.Substring(match.Index), "<([\\w:]+)[\\s+>]");
				...
				if (!string.IsNullOrEmpty(tagName))
				{
					num2 = HtmlParser.FindEndTag(stringToSearch, tagName, match.Index, out num);
				}
				if (num2 >= 0)
				{
		...
		public static Match MatchStartTag(string stringToSearch, string tagName, string attributeName, string attributeValue)
		{
			string text;
			return HtmlParser.MatchStartTag(stringToSearch, tagName, attributeName, attributeValue, out text);
		}
		...
		public static Match MatchStartTag(string stringToSearch, string tagName, string attributeName, string attributeValue, out string parsedAttributeValue)
		{
			...
			Match match = Regex.Match(stringToSearch, pattern);
