...
namespace Microsoft.Exchange.Connections.Eas.WBXml
{
	...
	internal class WBXmlStreamHelper
	{
		internal static uint UpdateCrc32(uint crc32, byte[] buffer, int offset, int length)
		{
			crc32 ^= uint.MaxValue;
			while (--length >= 0)
			{
				crc32 = (WBXmlStreamHelper.crcTable[(int)((UIntPtr)((crc32 ^ (uint)buffer[offset++]) & 255U))] ^ crc32 >> 8);
			}
			crc32 ^= uint.MaxValue;
			return crc32;
		}

		internal static int CopyStreamWithBase64Conversion(Stream source, Stream target, int byteCount, bool encode)
		{
			if (byteCount == 0)
			{
				return 0;
			...
			byte[] array = null;
			WBXmlStreamHelper.CharArrayBuffer charArrayBuffer = null;
			int num = 0;
			int num2 = 0;
			int result;
			try
			{
				array = WBXmlStreamHelper.BytesBufferPool.Acquire();
				charArrayBuffer = WBXmlStreamHelper.CharsBufferPool.Acquire();
				if (array == null)
				{
			...
			finally
			{
				if (array != null)
				{
					WBXmlStreamHelper.BytesBufferPool.Release(array);
				}
				if (charArrayBuffer != null)
				{
					charArrayBuffer.Clear();
					WBXmlStreamHelper.CharsBufferPool.Release(charArrayBuffer);
				}
			}
			return result;
		}
		...
		internal static int CopyStream(Stream source, Stream target, Encoding encoding, int charCount, bool needCrc, out uint crc)
		{
			...
			try
			{
				...
				do
				{
					...
					if (WBXmlStreamHelper.NeedToHandleSurrogate(charArrayBuffer.Buffer, num2, num3))
					{
						...
						if (num4 != -1)
						{
							...
							{
								string.Format(CultureInfo.InvariantCulture, "Invalid surrogate chars: first={0}, second={1}", new object[]
								{
									c2,
									c
								});
					...
					if (needCrc)
					{
						crc = WBXmlStreamHelper.UpdateCrc32(crc, array, 0, bytes);
					}
				}
				while (num < charCount);
				result = num;
		...
		internal static int CopyStream(StreamAccessor source, Stream target)
		{
			return WBXmlStreamHelper.CopyStream(source, target, int.MaxValue);
		}

		internal static int CopyStream(StreamAccessor source, Stream target, int byteCount)
		{
			uint num;
			return WBXmlStreamHelper.CopyStream(source, target, 0, byteCount, false, out num);
		}

		internal static int CopyStream(StreamAccessor source, Stream target, int byteCount, out uint crc)
		{
			return WBXmlStreamHelper.CopyStream(source, target, 0, byteCount, true, out crc);
		}

		internal static int CopyStream(StreamAccessor source, Stream target, int offset, int count)
		{
			uint num;
			return WBXmlStreamHelper.CopyStream(source, target, offset, count, false, out num);
		}

		internal static int CopyStream(StreamAccessor source, Stream target, int offset, int count, bool needCrc, out uint crc)
		{
			byte[] array = null;
			int num = 0;
			crc = 0U;
			...
			try
			{
				...
				do
				{
					...
					if (num2 != 0)
					{
						...
						if (needCrc)
						{
							crc = WBXmlStreamHelper.UpdateCrc32(crc, array, 0, num2);
						}
						count -= num2;
						num += num2;
					}
		...
		private static readonly BufferPool BytesBufferPool = new BufferPool(30720, EasConstants.MaxWorkerThreadsPerProc);

		private static readonly ThrottlingObjectPool<WBXmlStreamHelper.CharArrayBuffer> CharsBufferPool = new ThrottlingObjectPool<WBXmlStreamHelper.CharArrayBuffer>(Environment.ProcessorCount, EasConstants.MaxWorkerThreadsPerProc * Environment.ProcessorCount);
